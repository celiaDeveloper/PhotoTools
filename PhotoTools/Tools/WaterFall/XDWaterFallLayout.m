...
/** 默认参数 */
static const CGFloat XDDefaultColumnCount = 3; // 列数
static const CGFloat XDDefaultRowMargin = 10; // 行间距
static const CGFloat XDDefaultColumnMargin = 10; // 列间距
static const UIEdgeInsets XDDefaultEdgeInsets = {10, 10, 10, 10}; // edgeInsets

@interface XDWaterFallLayout ()
{
    struct { // 记录代理是否响应选择
...
@implementation XDWaterFallLayout
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    __block CGFloat minHeight = self.columnHeights.count ? [self.columnHeights[0] floatValue] : MAXFLOAT;
...
{
    return _delegateFlags.didRespondColumnCount ? [self.delegate columnCountOfWaterFallLayout:self] : XDDefaultColumnCount;
}

- (CGFloat)columnMargin
{
    return _delegateFlags.didRespondColumnMargin ? [self.delegate columnMarginOfWaterFallLayout:self] : XDDefaultColumnMargin;
}

- (CGFloat)rowMargin
{
    return _delegateFlags.didRespondRowMargin ? [self.delegate rowMarginOfWaterFallLayout:self] : XDDefaultRowMargin;
}

- (UIEdgeInsets)edgeInsets
{
    return _delegateFlags.didRespondEdgeInsets ? [self.delegate edgeInsetsOfWaterFallLayout:self] : XDDefaultEdgeInsets;
}
